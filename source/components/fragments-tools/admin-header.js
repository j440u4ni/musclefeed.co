      ...
      <React.Fragment>
          <Header style={{ background: '#ffffff', padding: 0, height: '36px' }} className="application-admin-header p-0">
            <div className="d-flex flex-row justify-content-end sub-admin-header p-0 align-items-center px-3">
              <span className="admin-information-first">{ this.state.user && this.state.user.firstname + ' ' + this.state.user.lastname }</span>
              <Button type={"danger"} onClick={this.onLogOut} shape="circle" className="ml-2 p-0"><Icon type="poweroff" className="button-icon-header" /></Button>
            </div>
          </Header>
      </React.Fragment>
    )
