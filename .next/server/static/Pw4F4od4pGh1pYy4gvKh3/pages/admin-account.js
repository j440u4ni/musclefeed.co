...
var retrieveAccount = function retrieveAccount(admin) {
  ...
  if (admin === true) {
    query = "mutation retrieveAdminAccount($email: String!, $password: String!) { retrieveAdminAccount(email: $email, password: $password) { firstname, lastname, token, authenticateToken } }";
  } else if (admin === false) {
    query = "mutation retrieveGuestAccount($email: String!, $password: String)! { retrieveGuestAccount(email: $email, password: $password) { firstname, lastname, token, authenticateToken } }";
  }
...
function authenticateService(email, password, admin) {
  ...
  return fetch('http://localhost:3000/v1/graphql-account', {
    method: 'POST',
    headers: {
...
// CONCATENATED MODULE: ./source/components/fragments-tools/alert-toasts.js
...
function authenticate(email, password, admin, reset) {
  return function (dispatch) {
    authenticateService(email, password, admin).then(function (first) {
      ...
      if (second.hasOwnProperty('errors')) {
        ...
        external_js_cookie_default.a.set('authentication-user', second.data.retrieveAdminAccount);
      }
    });
  };
...
function (_Component) {
  ...
  _createClass(SAdminAccount, [{
    ...
    value: function onIntentLogin() {
      ...
      dispatch(authenticate(this.state.email, this.state.password, true, this.onResetButton, link_default.a));
    }
  }, {
    key: "render",
    value: function render() {
